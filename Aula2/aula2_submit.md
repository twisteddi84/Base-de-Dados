...
```
As entidades são empresa, armazém (Atributos:número de produtos),produto(Atributos:Código,Nome,Taxa de IVA,Preço), Encomenda(Atributos:NºUnidades de um produto,número da encomenda,data,fornecedor),Fornecedor(Atributos:Nome,NIF,Condições de pagamento),Tipo de Fornecedor(Atributos:Designação do código interno). 
A empresa tem armazém. O armazém contêm produtos. O fornecedor tem um tipo. O fornecedor fornece encomenda. A encomenda contêm produtos.
```
...
```
Uma empresa tem N armazéns (grau binário). Um armazém contêm N produtos (grau binário). Um fornecedor tem um tipo de fornecedor e um tipo de fornecedor pode ter N fornecedores (grau binário). Obrigatoriadade: só existe tipo de fornecedor se existir fornecedores. Um fornecedor fornece N encomendas e uma encomenda só tem um fornecedor (grau binário). Obrigatoriedade: só existe encomenda se existir fornecedor. Uma encomenda contêm N produtos e um produto só pode estar em uma encomenda (grau binário). Obrigatoriedade: só existe encomenda se existir produto.
```
